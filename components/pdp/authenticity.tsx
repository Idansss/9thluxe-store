...
const STATUS_COPY: Record<PdpData["authenticity"]["status"], { label: string; detail: string }> = {
  RETAILER_INSPECTED: {
  ...
  MANUFACTURER_VERIFIED: {
    ...
    detail: "This product is verified through the manufacturer's own authentication process.",
  },
}
 ...
 */
export function Authenticity({ authenticity }: { authenticity: PdpData["authenticity"] }) {
  const status = STATUS_COPY[authenticity.status]
  const verifiedDate = authenticity.lastVerifiedAt
    ? new Date(authenticity.lastVerifiedAt).toLocaleDateString("en-NG", { year: "numeric", month: "long", day: "numeric" })
    : null
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1.2fr_1fr]">
      <div className="rounded-2xl border border-border bg-card p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm leading-relaxed text-muted-foreground">{status.detail}</p>
      ...
      <ul className="space-y-3 text-sm">
        <Guidance icon={<PackageCheck className="h-4 w-4" />} title="Check the seal">
          Bottles arrive factory-sealed in intact packaging. Inspect the cellophane and cap before first use.
        </Guidance>
        <Guidance icon={<ScanLine className="h-4 w-4" />} title="Inspect packaging & batch code">
          Compare the batch code on the box and bottle — they should match. Report any discrepancy to us.
        </Guidance>
        <Guidance icon={<LifeBuoy className="h-4 w-4" />} title="Report a concern">
          Something looks off? {""}
          <a href="/help#authenticity" className="text-accent underline-offset-2 hover:underline">
            Contact support
          </a>{" "}
...
function Guidance({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
    <li className="flex items-start gap-3 rounded-xl border border-border bg-secondary/40 p-3.5">
      <span className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-background text-accent">{icon}</span>
      <div>
